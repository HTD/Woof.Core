...
namespace Woof.Automation {
    ...
    /// <summary>
    /// <see cref="Uri"/> extension for finding download links to the latest version of software.
    /// </summary>
    public static class VersionLinkExtension {
        ...
        public static Uri FetchLastVersionLink(this Uri uri, params string[] patterns) {
        ...
        private static Uri GetLink(Uri uri, string pattern) {
            ...
            else link = html.IndexOf(pattern, IC2) > 0 ? pattern : null;
            if (link == null) return null;
            return new Uri(link.Contains("://") ? link : $"{uri.Scheme}://{uri.Host}{link}");
        }

        private const StringComparison IC2 = StringComparison.InvariantCultureIgnoreCase;
