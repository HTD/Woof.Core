...
namespace Woof.Automation {
    ...
    public class PostgresClientInstaller : IDisposable {
        ...
        static readonly string TargetPath = Path.Combine(PathTools.GetProgramFilesDirectory(PathTools.AutoTarget), "PGSQL");
        ...
        public bool Install() {
            ...
            if (Download(TargetPath)) {
                PathTools.AddToPath(TargetPath, PathTools.AutoTarget);
                return true;
            }
            else return false;
        }
        ...
        private bool Download(string targetPath) {
            ...
            var link = edb.FetchLastVersionLink(WindowsX64RelPattern, ZipArchivePattern);
            if (link == null) {
