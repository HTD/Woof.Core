...
namespace Woof.ProcessEx {
    ...
    /// Extended with advanced argument parser and ability to start process in current user context.
    /// Used to start processes interacting with user sessions from System account (Session0).
    /// </summary>
    public class ProcessEx : Process {
        ...
        /// <summary>
        /// If true, process will be created by user which has currently active logon session.
        /// </summary>
        public bool StartAsCurrentUser { get; set; }

        /// <summary>
        /// Returns this process or user process, whichever is applicable.
        /// Necessary when referencing process created with StartAsCurrentUser set.
        /// </summary>
        public Process ActualProcess => UserProcess ?? this;
        /// <summary>
        /// A collection of placeholders for process arguments (like ARG_A, ARG_B, ARG_C),
        /// which are set with <see cref="SetArg(string, string)"/> or <see cref="SetArgs(Dictionary{string, string})"/> methods.
        /// </summary>
        private Arguments ArgumentsTemplate;

        /// <summary>
        /// Creates a new process with specified executable path and command line arguments.
        /// </summary>
        /// <param name="path">Path to executable.</param>
        /// <param name="arguments">Command line arguments.</param>
        public ProcessEx(string path, params string[] arguments) : base() {
            var cd = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);
            StartInfo = new ProcessStartInfo {
                FileName = path,
                WorkingDirectory = Path.IsPathRooted(path) ? Path.GetDirectoryName(path) : Path.Combine(cd, Path.GetDirectoryName(path)),
                Arguments = (ArgumentsTemplate = new Arguments(arguments)).ToString(),
        ...
        /// <summary>
        /// Sets argument in process arguments collection to specified value.
        /// </summary>
        /// <param name="placeholder">Placeholder string defined in <see cref="ArgumentsTemplate"/>.</param>
        /// <param name="value">Value to set the argument to.</param>
        public void SetArg(string placeholder, string value) {
            var l = ArgumentsTemplate.Length;
            var n = new string[l];
            var a = String.Empty;
            for (int i = 0; i < l; i++) {
                a = ArgumentsTemplate[i];
                n[i] = a == placeholder ? value : a;
            }
            StartInfo.Arguments = new Arguments(n).ToString();
        }

        /// <summary>
        /// Sets multiple arguments in process arguments collection to values from specified dictionary.
        /// </summary>
        /// <param name="map">Dictionary containing placeholder => value pairs.</param>
        public void SetArgs(Dictionary<string,string> map) {
            ...
            for (int i = 0; i < l; i++) {
                a = ArgumentsTemplate[i];
                n[i] = map.ContainsKey(a) ? map[a] : a;
            }
            StartInfo.Arguments = new Arguments(n).ToString();
        }

        /// <summary>
        /// Starts the underlying <see cref="Process"/>,
        /// </summary>
        ...
        public new bool Start() => (StartAsCurrentUser && IsCurrentSystem)
                ? Win32_CreateProcessAsUser()
                : base.Start();
        ...
        public class Arguments {
            ...
            public Arguments(params string[] arguments) => Items = arguments;
        ...
        private bool Win32_CreateProcessAsUser() {
            if (UserProcess != null && !UserProcess.HasExited) return false;
            var command = Path.GetFileNameWithoutExtension(StartInfo.FileName);
            var cmdLine = command + ' ' + StartInfo.Arguments;
            ...
            try {
                ...
                var ok = NativeMethods.CreateProcessAsUser(
                    ...
                    ref StartupInfo, // A pointer to a STARTUPINFO or STARTUPINFOEX structure
                    out ProcessInformation // A pointer to a PROCESS_INFORMATION structure that receives identification information about the new process
                );
                iResultOfCreateProcessAsUser = Marshal.GetLastWin32Error();
                if (!ok) throw new InvalidOperationException($"CreateProcessAsUser failed, Win32 error: {iResultOfCreateProcessAsUser}.");
                UserProcess = GetProcessById((int)ProcessInformation.dwProcessId);
            }
            finally {
